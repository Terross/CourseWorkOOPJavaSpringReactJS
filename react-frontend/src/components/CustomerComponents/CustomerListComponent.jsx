...
export default function CustomerListComponent() {
  ...
  const [stateCustomers, setCustomersState] = React.useState([])
  ...
  const getCustomers=()=>{
      axios.get(CUSTOMER_API_BASE_URL).then(data=>{
  ...
  const deleteCustomers = id => {
     ...
     axios.delete(CUSTOMER_API_BASE_URL +'/'+id).then(data => {
       getCustomers();
     })
    ...
    <Container fixed={false} maxWidth={2440}>
      ...
      <Table stickyHeader aria-label="caption table">
        ...
        <TableBody>
          ...
          {stateCustomers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((emp) => {
                ...
                <TableCell align="center">
                      ...
                      <IconButton aria-label="delete"
                       className={classes.margin}
                        size="small"
                        onClick={() =>deleteCustomers(emp.id)}
                        >
